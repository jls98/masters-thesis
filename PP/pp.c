...
static void pp_monitor(Config *conf, Eviction_Set *evset, void *target) {
    ...
    // probe evset (cached)
    pp_probe(evset); // evset 7
    // load target (memory or hard drive)
    tar_buf[3] = probe(target);
    // probe evset (cached)
    pp_probe(evset); // evset 8
    // probe evset (cached)
    pp_probe(evset); // evset 9
    // probe target (L2/L3)
    tar_buf[4] = probe(target);
    // probe target (cached)
    tar_buf[5] = probe(target);
    // probe evset (cached)
...
static void pp_run(Config *conf, void *target_adrs) { // atm support only 1 adrs, extend later (easy w linked list)
	...
	pp_setup(conf, evset, cc_buffer, cc_buffer+4096); // just using cc buffer target rn for test purpose
