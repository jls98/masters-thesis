...
void test_test1(){
    ...
    for(int i=1;i<9;i+=1) evict_set_minimal = addElement(evict_set_minimal, i*(conf->cache_size/(conf->ways*conf->cache_line_size))); 
...
int main(int ac, char **av) {
	...
	if (ac==2){
		...
		if (conf_choice==22) conf = initConfig(8, 64, 58, 262144, 10000); 	// L2e i12 TODO
		if (conf_choice==23) conf = initConfig(8, 64, 58, 262144, 10000); 	// L3e i12 TODO or unneeded
		if (conf==NULL){
