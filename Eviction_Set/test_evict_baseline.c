...
void test_test1(){
    ...
    uint64_t c_size = conf->cache_size/4; // two times cache size but as uint64_t
	...
	*cand = 1;
    ...
    for (int i=0;i<100000;i++) CU_ASSERT_EQUAL(test1(cand_set[evict_set2->value], 3, cand, conf), 0); // assure self assignment
