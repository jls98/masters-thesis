...
void intern_access(Node **head1, Node **my_evset, u64 *msrmnt_, u64 index){
    for(int c=0;c<MSRMNT_CNT;c++){
        ...
        // flush evset    
        for(int i=0;i<EVSET_TARGETS;i++) flush(my_evset[i]); // change back to EVSET_TARGETS/25 if needed
...
void intern_access2(Node **head1, Node **my_evset, u64 *msrmnt_, u64 index, int evset_size){
    for(int c=0;c<MSRMNT_CNT;c++){
        
        // flush evset    
        for(int i=0;i<evset_size+1;i++) flush(my_evset[i]); // change back to EVSET_TARGETS/25 if needed
...
#define EVSET_L2_size 20
u64 replacement_L2_only_L2_complete(int evset_size){
    wait(1E9);
    Node *tmp=NULL;    // to hold tmp Nodes
    u64 index=0;    // holds index
    u64 size_stride_L2=2048; // holds current stride size in index value (size_stride*64 = X in Bytes)
    ...
    Node **buf_ptr=&buf;
    list_init(buf, buf_size);  
    Node **my_evset = malloc((evset_size+1)*sizeof(Node *));
    *my_evset=NULL;
    // create evsets manually and test them with targets
    conf = config_init(evset_size+1, 4096, 64, 120, 2097152, 1, 1); // L1
    index = 120*size_stride_L2 + offset;
    ...
    *head1 =NULL;
    // add 1st elem
    index = (evset_size-1+1)*size_stride_L2+offset; // -1
    tmp=list_take(buf_ptr, &index);
    list_append(head1, tmp);
    ...
    for (int i=0;i<evset_size+1;i++){
        intern_access2(head1, my_evset, msrmnt0+MSRMNT_CNT*i, i, evset_size);
    }
